...
const refreshToken = async () => {
  const response = (
    await axios.get('https://accounts.spotify.com/api/token', {
      params: {
...
export async function extractEpisodes(episodeIds) {
  ...
  for (let x = 0; x < chunkedEpisodes.length; x++) {
    ...
    episodesResult = await callSpotifyApi(
      async () =>
        (await spotifyApi.getEpisodes(chunkedEpisodes[x])).body.episodes
    );
...
export async function extractTrackAudioFeatures(trackIds) {
  return await callSpotifyApi(
    async () =>
      (await spotifyApi.getAudioFeaturesForTracks(trackIds)).body.audio_features
  );
}
